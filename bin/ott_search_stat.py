...
for query in queries:
    ...
    if response.status_code == 200:
        ...
        if type == "exact_search":
            ...
            all_commodities = sorted(goods_commodities + reference_commodities, key=lambda x: x["_score"], reverse=True)

            all_chapter_heading_results = sorted(reference_chapters + reference_headings, key=lambda x: x["_score"], reverse=True)

            all_chapter_heading_oter_results = sorted(goods_chapters + goods_headings, key=lambda x: x["_score"], reverse=True)

            commodity_codes = [
                get_goods_nomenclature_item_id(all_commodities[i]) if len(all_commodities) > i else "N/A"
                for i in range(5)
            ]

            results_codes = [
                all_chapter_heading_results[i]["_source"]["reference"]["goods_nomenclature_item_id"] if len(all_chapter_heading_results) > i else "N/A"
                for i in range(5)
            ]

            other_results_codes = [
                all_chapter_heading_oter_results[i]["_source"]["goods_nomenclature_item_id"] if len(all_chapter_heading_oter_results) > i else "N/A"
                for i in range(5)
            ]
